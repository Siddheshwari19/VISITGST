...
const CustomerRequests = (props) => {
    ...
    useEffect(() => {
        // setLoader(true);
        axiosInstance.get('/get-request-client').then((res) => {
                            ...
                            {loader ? <div style={{ display: 'flex', alignItems: 'center', height: '100vh', justifyContent: 'center' }} >
                                ...
                                <>
                                    {/* <Card style={{ borderRadius: '25px' }}> */}
                                        <h2 style={{borderRadius:'25px'}} className={classes.title}>Requests</h2>
                                    ...
                                    <Grid container spacing={3} justifyContent="space-between" >
                                        <Grid item lg={4} style={{ justifyContent: 'center', display: 'flex', alignItems: 'center' }}>
                                            <h3 style={{ margin: '1em' }}>Total Request: {count}</h3>
                                        </Grid>
                                        <Grid item style={{ justifyContent: 'center', display: 'flex' }} lg={4}>
                                            <TextField style={{ float: 'right', margin: '1em', width: "15em" }} color="secondary" variant="outlined" type="text" name="search" id="search" placeholder="Search requests" onChange={handleOnChange} />
                                    ...
                                    <Grid container>
                                        {stockData != undefined ?
                                            <Grid container >
                                                {stockData.map((appointment, index) => {
                                                    return (
